...
// Enhanced vertex shader with fresnel and better lighting
...
// Enhanced fragment shader with fresnel, realistic lighting, and glow
const sphereFragmentShader = `
  ...
  // 3D noise function for more realistic surface variation
  ...
  void main() {
    ...
    // Animated 3D noise for realistic energy surface
    float scale = isHub > 0.5 ? 2.5 : 3.5;
    ...
    // Energy flow pattern - more subtle for solid appearance
    float energyFlow = (noiseValue * 0.4 + 0.6) * (turbulence * 0.25 + 0.75);
    
    // Surface detail - less variation for solid look
    float surfaceDetail = energyFlow * 0.1 + 0.9;
    
    // Core color with subtle energy variation - more saturated
    vec3 baseColor = color * surfaceDetail;
    
    // Add slight color intensity boost for solid appearance
    baseColor = mix(baseColor, baseColor * 1.1, 0.3);
    
    // Inner core glow with energy flow
    vec3 coreGlow = glowColor * fresnelCore * 0.4 * (1.0 + energyFlow * 0.3);
    
    // Emissive glow that pulses
    float pulse = 1.0;
    if (isHub > 0.5 || selected > 0.5) {
...
export default function SphereShaderMaterial({
  ...
  const uniforms = useMemo(
    () => ({
      ...
      isHub: { value: isHub ? 1 : 0 },
    }),
    [color, glowColor, emissiveIntensity, opacity, isSelected, isHovered, isHub]
  );
